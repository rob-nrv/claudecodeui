  ...
  async function cleanupProject(projectPath, sessionId = null) {
    try {
      const externalProjectsRoot = await fs.realpath(
        path.join(claudeHomeFor(undefined, os.homedir()), 'external-projects')
      );
      ...
      if (sessionId) {
        try {
          const sessionPath = path.join(claudeHomeFor(undefined, os.homedir()), 'sessions', sessionId);
    ...
    try {
      ...
      if (githubUrl) {
        ...
        if (projectPath) {
          ...
          targetPath = path.join(claudeHomeFor(undefined, os.homedir()), 'external-projects', repoHash);
        }
